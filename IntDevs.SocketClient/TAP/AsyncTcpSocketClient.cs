...
namespace IntDevs.SocketClient
{
    public class AsyncTcpSocketClient
    {
        ...
        private static readonly ILog _log = log4net.LogManager.GetLogger("RNCloud.LogWarn");
        ...
        private Action<string> _msgUINotifyAction;
        ...
        public AsyncTcpSocketClient(IPEndPoint remoteEP, IAsyncTcpSocketClientMessageDispatcher dispatcher, AsyncTcpSocketClientConfiguration configuration ,Action<string> action)
            : this(remoteEP, null, dispatcher, configuration,action)
        {
        }

        public AsyncTcpSocketClient(IPEndPoint remoteEP, IPEndPoint localEP, IAsyncTcpSocketClientMessageDispatcher dispatcher, AsyncTcpSocketClientConfiguration configuration, Action<string> action)
        {
            if (remoteEP == null)
        ...
        private async Task Process()
        {
            try
            {
                ...
                while (State == TcpSocketConnectionState.Connected)
                {
                    ...
                    try
                    {
                        await _dispatcher.OnServerDataReceived(this, _receiveBuffer, _receiveBufferOffset, receiveCount, _msgUINotifyAction);
                    }
                    catch (Exception ex)
                    {
                        HandleUserSideError(ex);
                    ...
                    //        try
                    //        {
                    //            await _dispatcher.OnServerDataReceived(this, payload, payloadOffset, payloadCount,_msgUINotifyAction);
                    //        }
                    //        catch (Exception ex)
                    //        {
                    //            HandleUserSideError(ex);
